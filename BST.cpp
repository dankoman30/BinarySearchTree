...
void BST::PrivateAddNode(int key, node* pointer) {
	if (root == NULL) {
...
void BST::PrivatePrintInorder(node* pointer) { // inorder traversal (left, root, right)
...
void BST::PrivatePrintPreorder(node* pointer) { // preorder traversal (root, left, right)
	if (root != NULL) { // check to see if there's anything in the tree
		// root
		cout << pointer->key << " "; // print the current key
...
void BST::PrivatePrintPostorder(node* pointer) { // postorder traversal (left, right, root)
	if (root != NULL) { // check to see if there's anything in the tree
		// left
		if (pointer->left != NULL) { // if left is pointing to something, it's possible to traverse left
...
BST::node* BST::PrivateReturnNode(int key, node* pointer) {
...
void BST::PrintChildNodes(int key) { // print the two children of the specified key
...
int BST::PrivateFindMinimum(node* pointer) { // here, we are finding the smallest node in specified node's right subtree
...
int BST::PrivateFindMaximum(node* Ptr) { // here, we are finding the smallest node in specified node's right subtree
	if (root == NULL) { // if tree is empty
		cout << "EMPTY TREE!\n";
		return -666;
	}
...
void BST::RemoveKeyAtRoot() { // only used for removing the root node and reconstructing the tree
...
void BST::RemoveKey(node* parent, node* matchingNode, bool left) {
...
void BST::RemoveSubtree(node* pointer) { // post-order traversal to remove all the nodes
